...
void FastText::saveLabelVector() {
  std::ofstream ofs(args_->output + ".labels.vec");
  if (!ofs.is_open()) {
    std::cerr << "Error opening file for saving vectors." << std::endl;
    exit(EXIT_FAILURE);
  }
  ...
  Vector vec(args_->dim);
  for (int32_t i = dict_->nwords(); i < dict_->nwords() + dict_->nlabels(); i++) {
    std::string word = dict_->getWord(i);
    vec.zero();
...
void FastText::PVDM(
  ...
  for (int32_t w = 0; w < line.size(); w++) {
    ...
    bow.clear();
    bow.insert(bow.end(), labels.begin(), labels.end()); // PVDM
...
void FastText::PVDM(
  ...
  std::uniform_int_distribution<> uniform(1, args_->ws);
  for (int32_t w = 0; w < line.size(); w++) {
    int32_t wordid = dict_->getId(line[w]);
    if (wordid < 0) continue;

    int32_t boundary = uniform(model.rng);
    bow.clear();
    bow.insert(bow.end(), labels.begin(), labels.end()); // Distributed Memory

    std::vector<int32_t> transngrams;
    for (int32_t c = -boundary; c <= boundary; c++) {
      if (c != 0 && w + c >= 0 && w + c < line.size()) {
        const std::vector<int32_t>& ngrams = dict_->getSubwords(line[w + c]);
        transngrams.clear();
        transngrams.insert(transngrams.end(), ngrams.cbegin(), ngrams.cend());

        dict_->addWordNgrams(line, transngrams, w + c, args_->wordNgrams);
        bow.insert(bow.end(), transngrams.cbegin(), transngrams.cend());
      }
    }
    model.update(bow, wordid, lr);
  }

}

void FastText::predictPVDM(
        const std::vector<std::string>& line,
        std::shared_ptr<Matrix> labelMat,
        real lr) {
  if (line.size() == 0 || labelMat->m_ == 0 || labelMat->n_ == 0) return;
  ...
  for (int32_t w = 0; w < line.size(); w++) {
    ...
    model_->predictPVDM(bow, wordid, labelMat, lr);
  }
}
...
void FastText::rskipgram(Model& model, real lr,
                    const std::vector<int32_t>& line) {
  std::uniform_int_distribution<> uniform(1, args_->ws);
  for (int32_t w = 0; w < line.size(); w++) {
    int32_t boundary = uniform(model.rng);
    for (int32_t c = -boundary; c <= boundary; c++) {
      if (c != 0 && w + c >= 0 && w + c < line.size()) {
        const std::vector<int32_t> ngrams = dict_->getSubwords(line[w + c]);
        model.update(ngrams, line[w], lr); // Reversal-Skip-gram
      }
    }
  }
...
void FastText::predictPVDM(std::istream& in, const std::string& output, int32_t k) {

  std::shared_ptr<Matrix> preLabels = std::make_shared<Matrix>(k, args_->dim);
  preLabels->zero();
  ...
  int64_t linetokens = dict_->getLine(in, words, labels);

  for (int i = 0; i < args_->epoch; i++) {
    if (words.empty()) return;

    real progress = real(i + 1) / args_->epoch;
    real lr = args_->lr * (1.0 - progress);
    if (lr < args_->lr * 0.001) lr = args_->lr * 0.001;

    predictPVDM(words, preLabels, lr);
  ...
  if (!ofs.is_open()) {
    std::cerr << "Error opening file for saving predicted labels' vectors." << std::endl;
    exit(EXIT_FAILURE);
  }
  for (int32_t i = 0; i < preLabels->m_; i++) {
    for (int32_t j = 0; j < preLabels->n_; j++) {
      ofs << preLabels->at(i, j) << " ";
    }
    ofs << std::endl;
  }
  ofs.close();
...
void FastText::trainThread(int32_t threadId) {
  ...
  while (tokenCount < args_->epoch * ntokens) {
    ...
    real lr = args_->lr * (1.0 - progress);
    if (lr < args_->lr * 0.0001) lr = args_->lr * 0.0001; // minlr
    if (args_->model == model_name::sup) {
    ...
    } else if (args_->model == model_name::rsg) {
      localTokenCount += dict_->getLine(ifs, line, model.rng);
      rskipgram(model, lr, line);
    } else if (args_->model == model_name::PVDM) {
      localTokenCount += dict_->getLine(ifs, strline, labels);
      PVDM(model, lr, strline, labels);
    }
...
void FastText::train(std::shared_ptr<Args> args) {
  ...
  if (args_->pretrainedVectors.size() != 0) {
    ...
    input_ = std::make_shared<Matrix>(dict_->nwords()+dict_->nlabels()+args_->bucket, args_->dim);
    input_->uniform(1.0 / args_->dim);
  } else {
    input_ = std::make_shared<Matrix>(dict_->nwords()+args_->bucket, args_->dim);
    input_->uniform(1.0 / args_->dim);

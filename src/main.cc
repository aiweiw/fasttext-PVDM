...
void printPredictPVDMUsage() {
    std::cerr
      ...
      << "  <k>          (optional; 1 by default) predict number k labels\n"
      << std::endl;
}
...
void predictPVDM(const std::vector<std::string>& args) {
  if (args.size() < 5 || args.size() > 6) {
    printPredictPVDMUsage();
    exit(EXIT_FAILURE);
  }
  int32_t k = 1;
  if (args.size() >= 6) {
  ...
  fasttext.predictPVDM(ifs, std::string(args[4]), k);
  ifs.close();
...
void PVDM(const std::vector<std::string> args) {
  std::shared_ptr<Args> a = std::make_shared<Args>();
  a->parseArgs(args);
  FastText fasttext;
  fasttext.train(a);

    ...
    for (size_t j = i, n = 1; j < word.size() && n <= args_->maxn; n++) {
      ...
      if (n >= args_->minn && !(n == 1 && (i == 0 || j == word.size()))) {
        ...
        if (args_->model == model_name::PVDM) {
            ngrams.push_back(nwords_ + nlabels_ + h);
        } else {
...
void Dictionary::initNgrams() {
  for (size_t i = 0; i < size_; i++) {
    ...
    words_[i].subwords.push_back((int32_t)i);
...
void Dictionary::addWordNgrams(const std::vector<std::string>& line,
                               std::vector<int32_t>& transline,
                               int32_t i,
                               int32_t n) const {
  if (n <= 1 || i + 1 >= line.size()) return;
  uint64_t hi = hash(line[i]);
  for (int32_t j = i + 1; j < line.size() && j < i + n; j++) {
    uint64_t h = hi * 116049371 + hash(line[j]);
    pushHash(transline, h % args_->bucket);
  }
}
...
int32_t Dictionary::getLine(std::istream& in,
  ...
  while (readWord(in, token)) {
    ...
    if (type == entry_type::word) {
      ...
      word_hashes.push_back((int32_t)h);

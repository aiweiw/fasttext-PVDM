...
real Model::binaryLogistic(int32_t target, bool label, real lr, bool bup) {
...
real Model::negativeSampling(int32_t target, real lr, bool bup) {
  real loss = 0.0;
  grad_.zero();
  for (int32_t n = 0; n <= args_->neg; n++) {
    if (n == 0) {
      loss += binaryLogistic(target, true, lr, bup);
    } else {
      loss += binaryLogistic(getNegative(target), false, lr, bup);
    }
  }
  return loss;
}

real Model::hierarchicalSoftmax(int32_t target, real lr, bool bup) {
  real loss = 0.0;
  grad_.zero();
  const std::vector<bool>& binaryCode = codes[target];
  const std::vector<int32_t>& pathToRoot = paths[target];
  for (int32_t i = 0; i < pathToRoot.size(); i++) {
    loss += binaryLogistic(pathToRoot[i], binaryCode[i], lr, bup);
  }
  return loss;
}
...
void Model::computeHidden(const std::vector<int32_t> &input,
                          std::shared_ptr<Matrix> labelMat, Vector &hidden) const {
  assert(hidden.size() == hsz_);
  assert(labelMat->n_ == args_->dim);
  hidden.zero();
  for (auto it = input.cbegin(); it != input.cend(); ++it) {
    if (quant_) {
      hidden.addRow(*qwi_, *it);
  ...
  for (int32_t i = 0; i < labelMat->m_; i++) {
    hidden.addRow(*labelMat, i);
  }
  hidden.mul(1.0 / (input.size() + labelMat->m_));
}

void Model::predictPVDM(const std::vector<int32_t>& input, int32_t target,
                        std::shared_ptr<Matrix> labelMat, real lr) {
  assert(target >= 0);
  assert(target < osz_);
  if (input.size() == 0) return;
  computeHidden(input, labelMat, hidden_);
  if (args_->loss == loss_name::ns) {
    loss_ += negativeSampling(target, lr, false);
  } else if (args_->loss == loss_name::hs) {
    loss_ += hierarchicalSoftmax(target, lr, false);
  }
  nexamples_ += 1;

  if (args_->model == model_name::PVDM) {
    grad_.mul(1.0 / (input.size() + labelMat->m_));
  }

  for (int32_t i = 0; i < labelMat->m_; i++) {
    labelMat->addRow(grad_, i, 1.0);
  }
}
